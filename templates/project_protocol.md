# Bgent Project Protocol — 状态驱动工作流参考

<!--
  本文件定义项目级的状态驱动工作流。
  可以将这些规则整合进你的项目 README.md，或作为独立协议文件引用。
...
> 直接读取全文仅限：(a) 结项仪式需要回写时、(b) 脚本执行失败时、(c) 主动检索特定信息时。
...
3. **记忆加载**：读取 `.agent/memory/` 下今天和昨天的日志（由脚本纳入简报）
...
当项目包含多个场景时，你可以定义一个路由表，让 Agent 在进入特定任务时自动加载对应的文档：
...
2. **运行结项脚本**：`scripts/daily_briefing.py --mode closing --project-root ./`
...
本框架使用双层记忆架构实现同项目内的跨对话上下文持久化：

| 层 | 文件 | 生命周期 | 内容 |
|---|------|---------|------|
| **Daily Log** | `.agent/memory/YYYY-MM-DD.md` | 短期（14 天后归档） | 对话上下文、决策、新事实 |
| **Long-term** | `.agent/MEMORY.md` | 持久 | 用户显式要求记住的偏好/事实/教训 |
| **Achievements** | `AGENT_STATE.md` | 滚动 7 天 | 任务完成结果 |

**生命周期管理**：
- **Daily Log**：14 天后归档到 `.agent/memory/archive/YYYY/MM/`，不删除
- **MEMORY.md**：每月审核一次（删除过时 / 提升至项目文档 / 保留观察）
